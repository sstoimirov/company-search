...
const handleProgressBar = (from) => {
  ...
  if (monthsPassed < 24) {
    return `linear-gradient(90deg, #DF2121 ${monthsPassed}%, white ${monthsPassed}%)`
  }
  if (monthsPassed < 55) {
    ...
    ?.forEach((person) => {
      person?.functions?.forEach((func) => {
        returnValue.push({
          Name: person?.name,
          Position: func?.title,
          From: func?.valid_from
            ? new Date(func?.valid_from).toLocaleDateString("en-US", {
                year: "numeric",
                month: "short",
                day: "numeric",
              })
          ...
          Seniority: func?.valid_from ? (
            <div>
              <div>
                {monthDiff(new Date(func?.valid_from)) >= 12
                  ? `${Math.round(
                      monthDiff(new Date(func?.valid_from)) / 12,
                    )} year(s)`
                  : `${monthDiff(new Date(func?.valid_from))} months`}
              </div>
              <div
                style={{
          ...
          SeniorityOrder: new Date(func?.valid_from).toLocaleDateString(
            "en-US",
            {
              year: "numeric",
              month: "short",
